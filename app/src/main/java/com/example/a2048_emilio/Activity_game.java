package com.example.a2048_emilio;
...
import com.example.a2048_emilio.beans.Grid;
import com.example.a2048_emilio.beans.OnSwipeTouchListener;
import com.example.a2048_emilio.beans.Square;

import static com.example.a2048_emilio.beans.Square.context;

public class Activity_game extends AppCompatActivity {
    public static final String PREF_NAME = "Preferences";
    public static final int SWIPE_TRESHOLD
   ...
   Button restart;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(View v:viewsId){
            v.setOnTouchListener(new OnSwipeTouchListener(Activity_game.this){
...
float vx, float vy) {
        ...
        if (Math.abs(diffX) > Math.abs(diffY)) {
            if(Math.abs(diffX) > SWIPE_TRESHOLD && Math.abs(vx) > VELOCITY_TRESHOLD){
                if(diffX > 0){
                    grid.swipeOnGrid(Grid.RIGHT);
                }else {
